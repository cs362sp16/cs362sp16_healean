 ...
 * Description:
 *      Testing card effect Great_hall
 */
...
int main(int argc, char **argv) {
    ...
    // Testing parameters
    int card = great_hall; //Set the flag for the current card
    int choice1 = -1;
    int choice2 = -1;
    int choice3 = -1;
    int handPos = -1;
    ...
    for (i = MIN_PLAYERS; i < MAX_PLAYERS; i++) { // number of players
        for (j = 1; j < 10; j++) { // number of seeds
            ...
            int testNumActions = gamestate->numActions;

            // Testing great_hall
            testResult = cardEffect(card, choice1, choice2, choice3, gamestate, handPos, bonus);

            if (testResult == -1) {
                printf("cardEffect great_hall return: \n", testResult);
                overall = 1;
            }
            else if (testResult == 0) {
                if (gamestate->numActions != (testNumActions + 1)) {
                    printf("cardEffect great_hall: Failed \n");
                    overall = 1;
                }
            }
        }
